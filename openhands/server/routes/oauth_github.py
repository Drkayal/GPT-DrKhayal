...
async def github_oauth_callback(
    ...
    def _redirect_with_error(error_code: str) -> Response:
        url = f"{frontend_redirect}?oauth_error={urllib.parse.quote(error_code)}"
        resp = RedirectResponse(url=url)
        # Clear OAuth state cookie after failure
        resp.delete_cookie(STATE_COOKIE_NAME, path='/api/auth/github')
        return resp
    ...
    try:
        ...
        if not client_id or not client_secret:
            return _redirect_with_error('oauth_not_configured')
        ...
        async with httpx.AsyncClient() as client:
            ...
            if not access_token:
                return _redirect_with_error('no_access_token')
